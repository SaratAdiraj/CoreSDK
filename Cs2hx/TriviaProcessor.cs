...
namespace Cs2hx
{
	...
	{
		static ConcurrentBag<SyntaxTrivia> _triviaProcessed = new ConcurrentBag<SyntaxTrivia>();
		...
		public static void ProcessTrivias(HaxeWriter writer, IEnumerable<SyntaxTrivia> trivias)
		{
			...
			{
				//if (
				_triviaProcessed.Add(trivia);
				//) //ensure we don't look at the same trivia multiple times
				//{
		...
		{
			var triviaProcessed = new ConcurrentBag<SyntaxTrivia>();
